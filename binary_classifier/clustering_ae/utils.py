...
def encodeLDA(
    ...
    print('encoding LDA: LatentDirichletAllocation')
    #reshape array so that it works with sklearn
    train_features = train_features.reshape(-1, 1)
    #normalize all values to be between 0 and 1
    train_features = pre.MinMaxScaler().fit_transform(train_features)
    
    #reshape array so that it works with sklearn
    test_features = test_features.reshape(-1, 1)
    #normalize all values to be between 0 and 1
    test_features = pre.MinMaxScaler().fit_transform(test_features)

    lda = LDA(n_components=components)
    enc_train_features = lda.fit_transform(train_features)
